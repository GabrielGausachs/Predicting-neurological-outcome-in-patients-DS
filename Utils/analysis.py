...
class Analysis:
    ...
    def metrics(self):
        ...
        try:
            ...
            precision_good = precision_score(self.y_test, self.y_pred, zero_division=0) 
            recall_good = recall_score(self.y_test, self.y_pred, zero_division=0)  
            f1_good = f1_score(self.y_test, self.y_pred, zero_division=0)       
            precision_poor = precision_score(self.y_test, self.y_pred, pos_label=0, zero_division=0) 
            recall_poor = recall_score(self.y_test, self.y_pred, pos_label=0, zero_division=0) 
            f1_poor = f1_score(self.y_test, self.y_pred, pos_label=0, zero_division=0)
    ...
    def confusion_matrix(self):
        ...
        try:
            ...
            logger.info(f"  Metrics derived: Spec(Good)={specificity_good:.4f}, Spec(Poor)={recall_tpr_good:.4f}")
            return cm
        except Exception as e:
    ...
    def roc_curve_analysis(self, n_bootstraps=1000, alpha=0.95):
        ...
        # Implement ROC curve with confidence intervals
        try:
            y_scores = self.model.predict_proba(self.X_test)[:, 1]
        except Exception as e:
